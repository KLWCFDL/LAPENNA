...
/*
 *An example of allocating shared memory on the device. In this exercise, the
 *program copies global memory contents to shared memory, multiplies the
 *contents by 10, then stores it back to global memory
 *reference:https://cvw.cac.cornell.edu/gpu/shared_mem_exec
 */
