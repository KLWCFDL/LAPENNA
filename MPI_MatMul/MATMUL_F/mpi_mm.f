...
C AUTHOR: Ros Leibensperger / Blaise Barney. Converted to MPI: George L. 
C   Gusciora (1/23/95)
...
C******************************************************************************
     ...
     &          cols,avecol,extra, offset,i,j,k,ierr
      ...
      ! call MPI_INIT( ierr )
      call MPI_COMM_RANK( finfo%module_copy_comm, taskid, ierr )
      call MPI_COMM_SIZE( finfo%module_copy_comm, numtasks, ierr )
...
C     Send matrix data to the worker tasks 
     ...
     &                   dest, mtype, finfo%module_copy_comm, ierr )
          offset = offset + cols
 50     continue
...
C     Send results back to master task
     ...
     &                  mtype, finfo%module_copy_comm, ierr )
      endif
